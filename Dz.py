...
## 11)Decorator Pattern (Паттерн декоратора): позволяет добавлять поведение к
## объекту без изменения его класса.
## 12)Facade Pattern (Паттерн фасада): предоставляет упрощенный интерф
##ейс к сложной системе.
## 13)Flyweight Pattern (Паттерн летучей мыши): позволяет экономить
## ресурсы, разделяя общую информацию между объектами.
## 14)Proxy Pattern (Паттерн прокси): предоставляет контроль над доступом к
## объекту.
